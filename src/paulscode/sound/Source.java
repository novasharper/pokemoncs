...
public class Source {
	...
	/**
	 * Determines whether or not this is a priority source. Priority sources
	 * will not be overwritten by other sources when there are no available
	 * channels.
	 */
	...
	/**
	 * Either fade distance or rolloff factor, depending on the value of
	 * attModel.
	 */
	...
	/**
	 * This value represents the source's pitch (float value between 0.5f -
	 * 2.0f).
	 */
	...
	/**
	 * Specifies the gain factor used for the fade-out effect, or -1 when source
	 * is not currently fading out.
	 */
	protected float fadeOutGain = -1.0f;

	/**
	 * Specifies the gain factor used for the fade-in effect, or 1 when source
	 * is not currently fading in.
	 */
	...
	/**
	 * System time in miliseconds when the last fade in/out volume check
	 * occurred.
	 */
	 ...
	 *            the same sourcename.
	 * @param filenameURL
	 *            The filename/URL of the sound file to play at this source.
	 * @param soundBuffer
	 ...
	 * 
	 * @param filenameURL
	 *            The filename/URL of the sound file to stream next.
	 */
	 ...
	 * 
	 * @param filename
	 *            Filename/identifier of a sound file to remove from the queue.
	 */
	 ...
	 * simply fade out and stop. The miliseconds parameter must be non-negative
	 * or zero. This method will remove anything that is currently in the list
	 * of queued sounds that would have played next when the current sound
	 * finished playing. This method has no effect on non-streaming sources.
	 * 
	 * @param filenameURL
	 *            Filename/URL of the sound file to play next, or null for none.
	 * @param milis
	 ...
	 * fade-in completes will be equal to the source's previously assigned
	 ...
	 * remove anything that is currently in the list of queued sounds that would
	 * have played next when the current sound finished playing. This method has
	...
	/**
	 * Resets this source's volume if it is fading out or in. Returns true if
	 * this source is currently in the process of fading out. When fade-out
	 * completes, this method transitions the source to the next sound in the
	 * sound sequence queue if there is one. This method has no effect on
	...
	public boolean checkFadeOut() {
		...
		if (fadeOutGain >= 0.0f) {
			if (fadeOutMilis == 0) {
				...
				float fadeOutReduction = ((float) milisPast)
						/ ((float) fadeOutMilis);
		...
		if (fadeInGain < 1.0f) {
			...
			if (fadeInMilis == 0) {
				...
				float fadeInIncrease = ((float) milisPast)
						/ ((float) fadeInMilis);
	...
	/**
	 * Removes the next filename/URL from the sound sequence queue and assigns
	 * it to this source. This method has no effect on non-streaming sources.
	 * This method is used internally by SoundSystem, and it is unlikely that
	 * the user will ever need to use it.
	 * 
	...
	/**
	 * Returns the size of the sound sequence queue (if this is a streaming
	 * source).
	 * 
	...
	/**
	 * Sets whether or not this source should be removed when it finishes
	 * playing.
	 * 
	 * @param tmp
	 *            True or false.
	 ...
	 * will not be overritten by another source if there are no channels
	...
	/**
	 * Changes the sources peripheral information to match the supplied
	 * parameters.
	 * 
	 * @param priority
	 *            Setting this to true will prevent other sounds from overriding
	 ...
	 * @param c
	 *            Channel to stream on.
	 * @return Number of prior buffers that have been processed, or -1 if unable
	 *         to queue the buffer (if the source was culled, for example).
	 */
	...
	/**
	 * Stops and flushes the source, and prevents it from being played again
	 * until the activate() is called.
	 */
	public void cull() {
		if (!active(GET, XXX))
			return;
