...
public class MidiChannel implements MetaEventListener {
	...
	/**
	 * Specifies the gain factor used for the fade-in effect, or 1 when playback
	 ...
	 *            Unique identifier for this source.
	 * @param midiFile
	 *            URL to the MIDI file to play.
	 * @param identifier
	 *            Filename/identifier for the MIDI file.
	 */
	 ...
	 * @param sourcename
	 *            Unique identifier for this source.
	 * @param midiFilenameURL
	 *            Filename/URL to the MIDI file to play.
	 */
	...
	/**
	 * Initializes the sequencer, loads the sequence, and sets up the
	 * synthesizer.
	 */
	private void init() {
		// Load a sequencer:
	...
	/**
	 * Shuts the channel down and removes references to all instantiated
	 * objects.
	 */
	public void cleanup() {
		loading(SET, true);
	 ...
	 * the list of MIDI sequences to play when the previous sequence ends.
	 * 
	 * @param filename
	 *            Filename or identifier of a MIDI sequence to remove from the
	 *            queue.
	 */
	 ...
	 * begins playing the specified MIDI file at the previously assigned volume
	 * level. If the filenameURL parameter is null or empty, playback will
	 ...
	 * of queued MIDI sequences that would have played next when current
	 * playback finished.
	 * 
	 * @param filenameURL
	 *            MIDI file to play next, or null for none.
	 * @param milis
	 ...
	 * level. The filenameURL parameter may not be null or empty. The
	 * miliseconds parameters must be non-negative or zero. This method will
	 * remove anything that is currently in the list of queued MIDI sequences
	...
	/**
	 * Plays the MIDI file from the beginning, or from where it left off if it
	 * was paused.
	 */
	public void play() {
		if (!loading()) {
	 ...
	 *            Unique identifier for this source.
	 * @param filenameURL
	 *            Filename/URL of the MIDI file to play.
	 */
	...
	private void reset() {
		...
		if (sequencer == null) {
			errorMessage("Unable to set the sequence in method "
					+ "'reset', because there wasn't " + "a sequencer to use.");
			return;
		}
	 ...
	 *            GET or SET.
	 * @param value
	 *            New value if action == SET, or null if action == GET.
	 * @return The source's name.
	 */
	 ...
	 * 
	 * @param message
	 *            Meta mssage describing the MIDI event.
	 */
	...
	public void resetGain() {
		...
		int midiVolume = (int) (gain * SoundSystemConfig.getMasterGain()
				* (float) Math.abs(fadeOutGain) * fadeInGain * 127.0f);
		if (synthesizer != null) {
	...
	/**
	 * First attempts to load the specified "override MIDI synthesizer" if one
	 * was defined. If none was defined or unable to use it, then attempts to
	 * load the default synthesizer. If that fails, then other common
	 * synthesizers are attempted. If none can be loaded, then MIDI is not
	...
	private void getSynthesizer() {
		...
		String overrideMIDISynthesizer = SoundSystemConfig
	...
	/**
	 * Attempts to open the Sequencer with a name containing the specified
	 * string.
	 * 
	 * @param containsString
	 *            Part or all of a Sequencer's name.
	 ...
	 * 
	 * @param containsString
	 *            Part or all of a MIDI device's name.
	...
	private MidiDevice openMidiDevice(String containsString) {
		...
		for (int i = 0; i < midiDevices.length; i++) {
			...
			if (device != null
					&& midiDevices[i].getName().contains(containsString)) {
