...
public class Library {
	...
	/**
	 * Increments through the non-steaming channel list as new sources are
	 * played.
	 */
	 ...
	 * The 'super()' method should be at the top of constructors for all
	 * extended classes. The varriable 'libraryType' should be given a new value
	 * in the constructors for all extended classes.
	 */
	public Library() throws SoundSystemException {
		...
		streamingChannelSourceNames = new String[SoundSystemConfig
	...
	/**
	 * Stops all sources, shuts down sound library, and removes references to
	...
	public void rawDataStream(AudioFormat audioFormat, boolean priority,
			String sourcename, float posX, float posY, float posZ,
			int attModel, float distOrRoll) {
		sourceMap.put(sourcename, new Source(audioFormat, priority, sourcename,
				posX, posY, posZ, attModel, distOrRoll));
	}
	...
	public void newSource(boolean priority, boolean toStream, boolean toLoop,
			String sourcename, FilenameURL filenameURL, float posX, float posY,
			float posZ, int attModel, float distOrRoll) {
		sourceMap.put(sourcename, new Source(priority, toStream, toLoop,
				sourcename, filenameURL, null, posX, posY, posZ, attModel,
				distOrRoll, false));
	}
	...
	public void quickPlay(boolean priority, boolean toStream, boolean toLoop,
			String sourcename, FilenameURL filenameURL, float posX, float posY,
			float posZ, int attModel, float distOrRoll, boolean tmp) {
		sourceMap.put(sourcename, new Source(priority, toStream, toLoop,
				sourcename, filenameURL, null, posX, posY, posZ, attModel,
				distOrRoll, tmp));
	}
	...
	/**
	 * Sets the specified source's priority factor. A priority source will not
	 * be overriden if there are too many sources playing at once.
	 * 
	 * @param sourcename
	 *            The source's name.
	 * @param pri
	 ...
	 *            The source's name.
	 * @param dr
	 *            Fade distance or rolloff factor.
	 */
	...
	/**
	 * Culls the specified source. A culled source will not play until it has
	...
	/**
	 * Searches for and removes all temporary sources that have finished
	 * playing.
	 */
	...
	/**
	 * Returns a handle to the next available channel. If the specified source
	 * is a normal source, a normal channel is returned, and if it is a
	 * streaming source, then a streaming channel is returned. If all channels
	 * of the required type are currently playing, then the next channel playing
	 * a non-priority source is returned. If no channels are available (i.e.
	 * they are all playing priority sources) then getNextChannel returns null.
	 * 
	 * @param source
	 *            Source to find a channel for.
	 ...
	 * 
	 * @param toLoop
	 *            Midi file should loop or play once.
	 * @param sourcename
	 *            Source identifier.
	 * @param filenameURL
	 *            Filename/URL of the MIDI file to load.
	 */
	public void loadMidi(boolean toLoop, String sourcename,
		...
		if (!filenameURL.getFilename()
				.matches(SoundSystemConfig.EXTENSION_MIDI)) {
	...
	/**
	 * Indicates whether or not this library requires some codecs to
	 * reverse-order the audio data they generate.
	 * 
