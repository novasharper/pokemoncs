...
public class GuiTextField extends GuiElement {
	...
	public GuiTextField(int x, int y, Texture font, ButtonListener cb,
			GuiElement superior) {
		super(x, y, GraphicsConstants.LETTER_SIZE
				* (Constants.MAX_TEXT_LEN + 1) + 10,
	...
	public void drawBox() {
		if (visible) {
			Render2D.drawRect(new Location(x - 2, y - 2), width + 4,
					height + 4, new float[] { 0f, 0f, 0f }, false);
			Render2D.drawRect(new Location(x, y), width, height, new float[] {
					0.9f, 0.9f, 0.9f }, false);
			if (((GuiFrame) superior).hasFocus(this)) {
				Render2D.drawRect(new Location(Utilities.getStringWidth(text)
						+ 10 + x, GraphicsConstants.LETTER_SIZE + y + 3),
						GraphicsConstants.LETTER_SIZE - 3, 2, new float[] { 0f,
								0f, 0f }, false);
			}
		}
	}
	...
	public void handleTextInput(char c, int i) {
		...
		} else if (text.length() < Constants.MAX_TEXT_LEN && c != 0) { // add
																		// character
